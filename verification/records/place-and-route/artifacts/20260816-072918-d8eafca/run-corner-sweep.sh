...
# request (netlist/io/constraints/seed identical to the committed
...
# individually.
#
# Floorplan note (issue #56): this script's `floorplan` block is
# *deliberately* not identical to flow/par-modexp.json's as of issue #56 --
# it carries a revised, mapping-only floorplan (utilization 55%,
# core_margin_um 2, vs. par-modexp.json's 35%/4) explored as issue #56's
# mapping-only step toward closing the slow-corner timing gap recorded in
# spec/decision-records/0002-slow-corner-timing-closure-and-mm-red-critical-path.md.
# flow/par-modexp.json itself is deliberately left byte-identical to its
# pre-#56 content: its content_hash is cited as a live provenance input by
# both verification/records/place-and-route/records/20260814-203901-c741877.md
# and verification/records/drc-lvs/records/20260815-013937-fa169a4.md (whose
# layout/modexp.gds + layout/modexp.def artifacts were produced from the
# *original* floorplan and are not regenerated by issue #56) -- changing
# that file's bytes would break both records' provenance-hash-freshness
# check (verification/check_records.py) for evidence this issue does not
# own. See verification/records/place-and-route/records/ (issue #56's
# record) for the fresh corner-sweep measurement this revised floorplan
# produces, and its explicit statement of what layout/ does and does not
# reflect.
#
# Usage:
